...
public class TurmaController {
  ...
  private void validarAlunoAutenticado() {
    if (!autenticacaoController.isAutenticado()
      ...
      throw new IllegalArgumentException(
          "Apenas alunos podem consultar disciplinas e turmas disponíveis.");
    }
  }
  ...
  private void validarChoqueProfessor(
      String idTurmaIgnorada, String idProfessor, List<BlocoHorario> horarios) {
  ...
  private void validarAntesDoInicio(Turma turma) {
    if (!LocalDate.now().isBefore(turma.getDataInicioAulas())) {
      throw new IllegalArgumentException(
          "Turma não pode ser alterada ou cancelada após o início das aulas.");
    }
  }
}
