...
class MatriculaControllerTest {
  ...
  void setUp() {
    ...
    turmaMatriculada =
        criarTurma("turma-1", periodoAtivo.getId(), DayOfWeek.MONDAY, "08:00", "10:00", false);
  }
  ...
  void impedeMatriculaEmTurmaComHorarioSobreposto() {
    Turma turmaConflitante =
        criarTurma("turma-2", periodoAtivo.getId(), DayOfWeek.MONDAY, "09:00", "11:00", false);
    Matricula existente = new Matricula("mat-1", aluno.getId(), turmaMatriculada.getId());
    MatriculaController controller =
        criarController(List.of(existente), List.of(turmaMatriculada, turmaConflitante));
  ...
  void permiteHorariosConsecutivosSemSobreposicao() {
    Turma turmaSeguinte =
        criarTurma("turma-2", periodoAtivo.getId(), DayOfWeek.MONDAY, "10:00", "12:00", false);
    MatriculaController controller =
        criarController(
  ...
  void permiteMesmoHorarioEmOutroDiaOuPeriodo() {
    Turma outroDia =
        criarTurma("turma-2", periodoAtivo.getId(), DayOfWeek.TUESDAY, "08:00", "10:00", false);
    Turma outroPeriodo =
        criarTurma("turma-3", outroPeriodoAtivo.getId(), DayOfWeek.MONDAY, "08:00", "10:00", false);
    MatriculaController controller =
        criarController(
            List.of(new Matricula("mat-1", aluno.getId(), turmaMatriculada.getId())),
  ...
  void turmaCanceladaJaMatriculadaNaoGeraConflito() {
    Turma cancelada =
        criarTurma(
            "turma-cancelada", periodoAtivo.getId(), DayOfWeek.MONDAY, "08:00", "10:00", true);
    Turma novaTurma =
        criarTurma("turma-nova", periodoAtivo.getId(), DayOfWeek.MONDAY, "08:00", "10:00", false);
    MatriculaController controller =
        criarController(
  ...
  void rejeitaTurmaIndisponivelInexistenteOuDuplicada() {
    Turma cancelada =
        criarTurma(
            "turma-cancelada", periodoAtivo.getId(), DayOfWeek.WEDNESDAY, "08:00", "10:00", true);
    MatriculaController controller =
        criarController(
            List.of(new Matricula("mat-1", aluno.getId(), turmaMatriculada.getId())),
    ...
    assertAll(
        ...
        () ->
            assertThrows(
                UnsupportedOperationException.class, () -> controller.getMatriculas().clear()));
  }

  private MatriculaController criarController(List<Matricula> matriculas, List<Turma> turmas) {
    return new MatriculaController(
        autenticacaoController, matriculas, turmas, List.of(periodoAtivo, outroPeriodoAtivo));
  }

  private Turma criarTurma(
      String id, String idPeriodo, DayOfWeek dia, String inicio, String fim, boolean cancelada) {
    return new Turma(
        id,
        "disc-" + id,
        idPeriodo,
